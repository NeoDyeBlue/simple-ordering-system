...
export async function getAllPhones(brandName, filter) {
  await dbConnect();
  let response = {
    brand: "",
    phones: [],
  ...
  try {
    if (brandName == "all") {
      const result = await Phone.paginate({}, options);
      response.phones = result.docs;
      response.brand = "all";
    } else {
      const brandItem = await Brand.findOne({ name: brandName })
        .select("name")
        .exec();
      if (brandItem) {
        response.brand = brandItem.name;
        const result = await Phone.paginate({ brand: brandItem._id }, options);
        response.phones = result.docs;
      }
    }
  } catch (err) {

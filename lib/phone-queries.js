...
export async function searchPhones(query, _options) {
  await dbConnect();
  let result = [];
  ...
  try {
    const data = await Phone.paginate(
      { name: { $regex: formatted, $options: "i" } },
      options
    );
    // console.log(data);
    result = data.docs;
...
export async function getAllPhones(_brand, _query, _options) {
  await dbConnect();
  let response = {
    brand: "all",
    phones: [],
  ...
  const brand = _brand.split("-").join(" ").toLowerCase();
  ...
  try {
    if (!brand || brand == "all") {
      ...
      const brandItem = await Brand.findOne({ name: brand })
        .select("name")
        .exec();
      if (brandItem) {
...
export async function getPhone(_brand, _model) {
  await dbConnect();
  let response = {};
  const brand = _brand.split("-").join(" ").toLowerCase();
  const model = _model.split("-").join(" ").toLowerCase();
  try {
    const brandItem = await Brand.findOne({ name: brand })
      .select("name")
      .exec();
    const phone = await Phone.findOne({
      $and: [{ brand: brandItem._id }, { name: model }],
    });
